...
@interface gfxCardStatusAppDelegate ()
@property (nonatomic, strong) PowerSensor* ps;
...
//    [logger setFilter:[[GTMLogNoFilter alloc] init]];
    ...
    self.ps = [[PowerSensor alloc] init];
    [self.ps start];
    [[[[self.ps rac_valuesForKeyPath:@"value" observer:self] map:^id(id value) {
        NSDictionary* dic = (NSDictionary*)value;
        if ([dic isKindOfClass:[NSDictionary class]] && [value[@"value"] isKindOfClass:[NSNumber class]]) {
            NSNumber* val = value[@"value"];
            return val;
        ...
        if ([x isKindOfClass:[NSNumber class]]) {
            NSNumber* val = x;
            if([val intValue] == kBattery) {
                [GSMux setMode:GSSwitcherModeDynamicSwitching];
            }else if ([val intValue] == kAC){
                [GSMux setMode:GSSwitcherModeForceDiscrete];
            }
...
//        NSLog(@"Check for updates on startup value changed: %@", x);
...
- (void)applicationWillTerminate:(NSNotification *)aNotification
{
    ...
    NSLog(@"Termination notification received. Going to Dynamic Switching.");
}
